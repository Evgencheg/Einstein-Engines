...
namespace Content.Client.Preferences.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
        ...
        private void UpdateGenderControls()
        {
            ...
            Image canvas = new Image<Rgba32>(EyeManager.PixelsPerMeter, EyeManager.PixelsPerMeter);
            EntityUid dummy = (EntityUid) _controller.GetPreviewDummy()!;
            EntityData entity = new EntityData(dummy, _entityManager.GetComponent<SpriteComponent>(dummy), 0, 0);
            Run(canvas, entity, _entityManager.System<SharedTransformSystem>());
            canvas.SaveAsPng("test.png");
        }
        ...
        private readonly Dictionary<(string path, string state), Image> _images = new();
        void Run(Image canvas, EntityData entity, SharedTransformSystem xformSystem)
        {
            Image _errorImage = Image.Load<Rgba32>(_resManager.ContentFileRead("/Textures/error.rsi/error.png"));
            ...
            foreach (var layer in entity.Sprite.AllLayers)
            {
                ...
                {
                    Console.WriteLine($"Invalid layer {rsi!.Path}/{layer.RsiState.Name}.png for entity {_entityManager.ToPrettyString(entity.Owner)} at ({entity.X}, {entity.Y})");
                    return;
                }
